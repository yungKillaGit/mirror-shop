...
**Tests**: The examples below include test tasks. Tests for behavior changes are REQUIRED by
the constitution; if a change has no behavior impact, document the rationale in tasks.md.
...
- [ ] T004 [P] Confirm FSD slice map, public APIs, and App Router adapter boundaries
...
- [ ] T010 Update src/shared/config/env-config.ts for any new environment contract
- [ ] T011 Define audit trail events for listing, offer, and settlement lifecycle actions
- [ ] T012 Define performance and bundle budgets for affected flows/pages
...
- [ ] T021 [US1] Add anti-scam checks and explicit trade term confirmation in UX/API
- [ ] T022 [US1] Verify responsive behavior and accessibility for user story 1 flow
...
- [ ] TXXX Validate FSD import boundaries and public API usage (no forbidden deep imports)
- [ ] TXXX Validate mobile-responsive behavior on supported breakpoints
- [ ] TXXX Run accessibility checks for critical flows and resolve violations
- [ ] TXXX Validate no significant performance degradation and no unjustified bundle growth
- [ ] TXXX Validate anti-scam controls and moderation/dispute tooling coverage
- [ ] TXXX Validate metadata, robots, and sitemap updates for public route changes
- [ ] TXXX Run npm run lint, npm run test, and npm run build; attach evidence in PR
